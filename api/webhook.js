...
async function fetchUserRequest(tgId) {
  if (!SHEETS_URL) return null;
  try {
    const res = await fetch(`${SHEETS_URL}?sheet=${encodeURIComponent('Запросы')}`);
    const json = await res.json();
    const rows = json.rows || [];
    return rows.find(r => String(r.tg_id) === String(tgId)) || null;
  } catch { return null; }
}
  ...
  try {
    ...
    if (body.callback_query) {
      ...
      if (action === 'grant') {
        ...
        // Short format: g|role|tgId (generated by Auth.jsx)
        const [, role, tgId] = parts;
        const numId    = Number(tgId);
        ...
        // Short format: d|tgId (generated by Auth.jsx)
        const [, tgId] = parts;
        const numId    = Number(tgId);
        const userData = await fetchUserRequest(tgId);
        const firstName = userData?.first_name || '';
