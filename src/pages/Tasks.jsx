...
// Общий стиль для input-полей задач
...
// ─────────────────────────────────────────────────────────────────────────────
//  Все компоненты — НА УРОВНЕ МОДУЛЯ.
//  Если определить их внутри Tasks(), React пересоздаёт тип на каждый рендер
//  → перемонтирует поля → клавиатура закрывается после 1 символа.
// ─────────────────────────────────────────────────────────────────────────────
...
function TaskCard({ task, compact, setModal }) {
...
function FAB({ defaultCol, setCreateCol, setForm }) {
  return (
    <motion.button
...
function MobileView({ cols, tasks, activeCol, setActiveCol, setModal, setCreateCol, setForm }) {
  return (
    <div>
      ...
      {cols.filter(c => c.id === activeCol).map(col => {
        ...
        return (
          <div key={col.id} style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {colTasks.map(task => <TaskCard key={task.id} task={task} setModal={setModal} />)}
            {colTasks.length === 0 && (
      ...
      <FAB defaultCol={activeCol} setCreateCol={setCreateCol} setForm={setForm} />
    </div>
  );
}

function DesktopView({ cols, tasks, setModal, setCreateCol, setForm }) {
  return (
        ...
        return (
          <div key={col.id} style={{ background:COL_STYLE[col.id].bg, borderRadius:16, padding:12, border:'1px solid #e8f4f6' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {colTasks.map(task => <TaskCard key={task.id} task={task} compact setModal={setModal} />)}
            </div>
          </div>
        );
      })}
...
function CreateModal({ createCol, setCreateCol, form, setForm, createTask }) {
  return (
    <AnimatePresence>
      {createCol && (
...
function TaskModal({ modal, setModal, comment, setComment, tag, setTag, fileRef,
  user, other, cols, addComment, markRead, changePri, changeDate, localMoveTask,
  toggleSub, addSub, removeSub, editSubText, handleFile }) {
...
export default function Tasks({ user }) {
  ...
  const addComment = () => {
    ...
    patch(modal.id, { comments: [...modal.comments, entry] });
    setComment(''); setTag(false);
  };

  const markRead = (taskId, ci) => {
    const now = new Date().toLocaleTimeString('ru-RU', { hour:'2-digit', minute:'2-digit' });
    const t = tasks.find(t=>t.id===taskId); if (!t) return;
    patch(taskId, { comments: t.comments.map((c,i) => i===ci ? c.replace('|unread',`|✓✓ ${now}`) : c) });
  };

  const localMoveTask = (id, status) => { moveTask(id, status); setModal(t => t?.id===id ? {...t,status} : t); };
  const toggleSub   = (id, si) => { const t=tasks.find(t=>t.id===id); if(!t)return; patch(id,{subtasks:t.subtasks.map((s,i)=>i===si?{...s,done:!s.done}:s)}); };
  const removeSub   = (id, si) => { const t=tasks.find(t=>t.id===id); if(!t)return; patch(id,{subtasks:t.subtasks.filter((_,i)=>i!==si)}); };
  const addSub      = (id)     => { const t=tasks.find(t=>t.id===id); if(!t)return; patch(id,{subtasks:[...t.subtasks,{t:'',done:false}]}); };
  const editSubText = (id, si, v) => { const t=tasks.find(t=>t.id===id); if(!t)return; patch(id,{subtasks:t.subtasks.map((s,i)=>i===si?{...s,t:v}:s)}); };
  ...
  const handleFile = e => {
    ...
    const now = new Date().toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'});
    patch(modal.id, { comments: [...modal.comments, `${user.name.split(' ')[0]}: 📎 ${f.name}|${now}|✓✓ прикреплён`] });
  };
  ...
  const createTask = () => {
    ...
    const newTask = {
      ...
      due: form.due || '—', subtasks: form.subtasks.filter(s => s.t.trim()).map(s => ({ t:s.t.trim(), done:false })),
      comments: [],
    };
    addTask(newTask);
  ...
  const modalProps = {
    modal, setModal, comment, setComment, tag, setTag, fileRef,
    user, other, cols,
    addComment, markRead, changePri, changeDate, localMoveTask,
    toggleSub, addSub, removeSub, editSubText, handleFile,
  };

  return (
    <div className="space-y-4">
      <CreateModal createCol={createCol} setCreateCol={setCreateCol} form={form} setForm={setForm} createTask={createTask} />
      <TaskModal {...modalProps} />
      <div className="lg:hidden">
        <MobileView cols={cols} tasks={tasks} activeCol={activeCol} setActiveCol={setActiveCol}
          setModal={setModal} setCreateCol={setCreateCol} setForm={setForm} />
      </div>
      <div className="hidden lg:block">
        <DesktopView cols={cols} tasks={tasks} setModal={setModal} setCreateCol={setCreateCol} setForm={setForm} />
        <FAB defaultCol="todo" setCreateCol={setCreateCol} setForm={setForm} />
      </div>
    </div>
  );
}
